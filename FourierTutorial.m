...
% Answer: even -> real, odd -> imaginary, neither -> both
...
% offset to the right by k. Can you see this? 
...
b = exp(-t/tau);
% s1 = zeros(size(t));
% s1(find(mod(t,0.2) < 0.1))=5;
% b = 1/(sqrt(2.*pi).*tau1).*exp(-((t-.5)/tau1).^2)...
%     -1/(sqrt(2.*pi).*tau2).*exp(-((t-.5)/tau2).^2);
% A = tril(toeplitz(b));
...
% g1 = fftshift(exp(-((t-.5)/tau).^2));
% g1 = 1/(sqrt(2.*pi).*tau).*exp(-((t-.5)/tau).^2)-1/(sqrt(2.*pi).*2.*tau).*exp(-((t-.5)/(2.*tau)).^2);
tau1 = tau.*0.25;
tau2 = tau.*0.5;
g1 = 1/(sqrt(2.*pi).*tau1).*exp(-((t-.5)/tau1).^2)...
    -1/(sqrt(2.*pi).*tau2).*exp(-((t-.5)/tau2).^2);
g2 = fftshift(g1);
G1 = fft(g1);
G2 = fft(g2);
...
plot(t,g2)
subplot(6,2,7)
plot(fax,(fftshift(real(G1))))
subplot(6,2,8)
plot(fax,(fftshift(real(G2))))
subplot(6,2,9)
plot(t,ss1(1:length(t)))
subplot(6,2,10)
plot(t,ss2)
subplot(6,2,11)
plot(fax, fftshift(abs(SS1)))
subplot(6,2,12)
plot(fax, fftshift(abs(SS2)))
