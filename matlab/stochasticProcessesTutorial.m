...
% Make sure you've got plot1ras.m and plot2ras.m on your path
clear all
...
% We generated 300 spikes for each of the trials in our simulation.  
...
% This is sum(-log2(p) .* p). This quantity has units 'bits' and is related
...
% Homework problem 3 (optional). Write a differential equation to capture
...
s0 = [0.2 0.4 0.4]        % starting from closed
...
% steady state (about 45% of channels in open state; the exact value is
% 32/71 = 0.4507, confirmed by the eigenvector calculation further down).
%
...
% But the factorization of T^n contains a eigenvalues raised to the nth power.
...
%
% *************************************************************************
%% Finally, let's use the markov chain to analyze a simple random walk problem --
% a very simply approximation to diffusion, which is often referred to as a
% gambler's ruin. Suppose Fred and Adrienne each have 4 dollars. They each bet a
% dollar and then flip a coin. If it comes up heads, Fred wins a dollar from
% Adrienne. If it comes up tails, Adrienne wins a dollar from Fred. The game
% ends when one player has 8 dollars and the other has none. We can define 9
% states of this process in terms of the relative winnings. Fred ahead by 4,
% Fred ahead by 3, ..., Fred and Adrienne even, Adrienne ahead by 1, ...,
% Adrienne ahead by 4. Let's begin by assuming that the coin is fair.
p  = .6,  q = 1-p
...
% diagonal. The probability of leaving these states is zero; the game is over
% when Fred or Adrienne is ahead by 4. We call these absorbing states. For the
% intermediate states 2 through 8, there is no chance of staying in that state.
% There is a probability, p, that the next state will favor Adrienne and a
% probability q=1-p that the next state will favor Fred. The problem resembles
% the diffusion of a particle toward some absorbing barrier placed symmetrically
% on either side of the origin.

% Homework question. What is the probability distribution of states after 16
% rounds of play if the game begins in state 5 (both players have $4)? What is
% the probability that the game has ended by 16 rounds? Notice that the
% probability is 0 for some of the 9 states (if you did it right, the
% probability should be zero for 4 of the 9 states). Why is this? Now, suppose
% that the coin unfairly favors Adrienne. Choose p=0.6. What is the probability
% that the game has ended by round 16? What is the probability that it has ended
% with Adrienne the winner?
