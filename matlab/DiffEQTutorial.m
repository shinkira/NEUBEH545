...
%            1/09 FMR updated for 2009 class
%
%****************************************************************************
...
k = 0.1;
TimeStep = 0.001./k;		% time step for difference equation (in sec)
PrePts = 200.*k;			% points before step in y
StmPts = 400.*k;			% number of time points that y is active
NumPts = 1000.*k;			% total points to simulate
...
phi = 20;				% phosphodiesterase activity decay rate constant (1/sec)
...
% describing the strength of the feedback.  Otherwise the model is the
% same.

figure(2)
cdark = 0.5;			% dark calcium concentration
beta = 0.1;				% rate constant for calcium removal in 1/sec
% NOTE: 1/beta = 10 sec is the calcium time constant, but this simulation only
% runs for NumPts*TimeStep = 1 sec, so calcium barely moves and the feedback is
% effectively inactive.  Try beta = 10 (tau = 100 ms) to see the feedback work,
% and see Questions/Problems #5(3) below.
...
alpha = 200;				% inactive->active (activation) rate constant
beta = 100;				% active->inactive (deactivation) rate constant
NumPts = 1000;			
TimeStep = 0.00001;
...
% NOTE: the ./ here must be element-wise.  With a plain / MATLAB reads this as
% matrix right division (mrdivide), silently returns a single least-squares
% scalar, and alpha comes out CONSTANT with voltage - so the figure below shows
% no voltage-dependent activation at all.
